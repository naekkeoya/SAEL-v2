	   	...
	   	if ($sesion)
			{
			 ...
			 $query = mysqli_query($conexion, "SELECT *FROM cliente WHERE nombre_cliente = '$busqueda'");
			 $partner = mysqli_fetch_array($query);
			 if ($partner)
			 {
				 $query = mysqli_query($conexion, "SELECT *FROM evento WHERE id_cliente = '$partner[0]'");
				 $partner = mysqli_fetch_array($query);
				 if (!$partner)
				 {
					 header("location: search_event_delete_fail_search.php");
				 }
			 }
			 else
			 {
			 ...
			 <section class='packages pt-5'>
						 <h2 class='heading text-capitalize text-center'>Eliminar Evento</h2>
				 <div class='container py-lg-4 py-sm-3 col-sm-6 d-flex flex-row '>

					 <fieldset class='fieldset2 container align-items-center'>
						 <form action='redelete_event.php' method='POST' class='contact'>
						 <div class='form-group'>
							 <input type='text' class='form-control' id='ids' name='id' placeholder='$partner[0]' readonly='' value = '$partner[0]' required=''>
						 </div>
							 <div class='form-group'>
								 <input type='text' class='form-control' id='id_cliente' name='id_cliente' placeholder='$partner[1]' readonly='' value = '$partner[1]' required=''>
							 </div>
							 <div class='form-group'>
								 <input type='text' class='form-control' id='id_agente' name='id_agente' placeholder='$partner[2]' readonly='' value = '$partner[2]' required=''>
							 </div>
							 <div class='form-group'>
								 <input type='text' class='form-control' id='id_paquete' name='id_paquete' placeholder='$partner[3]' readonly='' value = '$partner[3]' required=''>
							 </div>
							 <div class='form-group'>
								 <input type='text' class='form-control' id='fecha' name='fecha' placeholder='$partner[4]' readonly='' value = '$partner[4]' required=''>
							 </div>
							 <div class='form-group'>
								 <input type='text' class='form-control' id='hora' name='hora' placeholder='$partner[5]' readonly='' value = '$partner[5]' required=''>
							 </div>
							 <div class='form-group'>
								 <input type='text' class='form-control' id='duracion' name='duracion' placeholder='$partner[6]' readonly='' value = '$partner[6]' required=''>
							 </div>
