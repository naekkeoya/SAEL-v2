	<?php
	    ...
	    if ($sesion)
		    ...
		    $tipo = $sesion[3];
				...
				$query = mysqli_query($conexion, "SELECT *FROM proveedor WHERE nombre_proveedor = '$busqueda'");
				$partner = mysqli_fetch_array($query);
				if (!$partner)
				{
						header("location: search_partner_delete_fail_search.php");
				}
				echo "<!DOCTYPE html>
		    ...
		    <section class='packages pt-5'>
		      		<h2 class='heading text-capitalize text-center'>Eliminar Proveedor</h2>
		    	<div class='container py-lg-4 py-sm-3 col-sm-6 d-flex flex-row '>

		        <fieldset class='fieldset2 container align-items-center'>
		          <form action='redelete_partner.php' method='POST' class='contact'>
							<div class='form-group'>
								<input type='text' class='form-control' id='ids' name='id' placeholder='$partner[0]' readonly='' value = '$partner[0]' required='' aria-describedby='ids'>
							</div>
		            <div class='form-group'>
		              <input type='text' class='form-control' id='nombres' name='nombre' placeholder='$partner[1]' readonly='' required='' aria-describedby='Nombre'>
		            </div>
		            <div class='form-group'>
		              <input type='text' class='form-control' id='telefonos' name='telefono' placeholder='$partner[2]' readonly='' required='' aria-describedby='Telefono'>
		            </div>
		            <div class='form-group'>
		              <input type='text' class='form-control' id='giros' name='giro' placeholder='$partner[3]' readonly='' required='' aria-describedby='Giro'>
		            </div>
		            <div class='form-group'>
		              <input type='email' class='form-control' id='correos' name='correo' placeholder='$partner[4]' readonly='' required=''>
		            </div>
								<div class='form-group'>
		              <input type='text' class='form-control' id='estados' name='estado' placeholder='$partner[5]' readonly='' required=''>
		            </div>
								<div class='form-group'>
		              <input type='text' class='form-control' id='ciudades' name='ciudad' placeholder='$partner[6]' readonly='' required=''>
		            </div>
								<div class='form-group'>
		              <input type='text' class='form-control' id='colonias' name='colonia' placeholder='$partner[7]' readonly='' required=''>
		            </div>
								<div class='form-group'>
		              <input type='text' class='form-control' id='calles' name='calle' placeholder='$partner[8]' readonly='' required=''>
		            </div>
								<div class='form-group'>
		              <input type='text' class='form-control' id='numero' name='numero' placeholder='$partner[9]' readonly='' required=''>
		            </div>
		            <button type='submit' id='addPartner' class='btn btn-primary btn-lg'> Eliminar</button>
		          </form>
		        </fieldset>
		      </div>
		    </section>
