<?php
    ...
    if ($sesion)
	    ...
	    <section class='packages pt-5'>
	      <h2 class='heading text-capitalize text-center'>Mostrar Eventos</h2>
	    	<div class='container py-lg-4 py-sm-3 col-sm-6 d-flex flex-row '>";
				...
				if (!mysqli_connect_error())
				{
					 ...
					 $datos = mysqli_query($conexion, "SELECT * FROM evento");
					 while ($fila = mysqli_fetch_array($datos))
						{
							 $cliente = mysqli_query($conexion, "SELECT * FROM cliente WHERE id_cliente = '$fila[1]'");
							 $cliente = mysqli_fetch_array($cliente);
							 $usuario = mysqli_query($conexion, "SELECT * FROM usuarios WHERE id_usuario = '$fila[2]'");
							 $usuario = mysqli_fetch_array($usuario);
							 $paquete = mysqli_query($conexion, "SELECT * FROM paquete WHERE id_paquete = '$fila[3]'");
							 $paquete = mysqli_fetch_array($paquete);
							 echo
							 "<tr>
							 	<td>$fila[0]</td>
							 	<td>$cliente[1]</td>
